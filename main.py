...
def export_template():
    ...
    if source_type == 'wgs84':
        ...
        template_data = pd.DataFrame({
            "经度": [118.182174, 117.000923, 120.335678],
            "纬度": [35.519406, 36.675807, 36.067890],
            "备注（可选）": ["示例点1", "示例点2", "示例点3"]
        })
        filename = 'WGS84转国家2000模板.xlsx'
    else:
        # CGCS2000→WGS84模板（新增）
        template_data = pd.DataFrame({
            "X坐标": [38521456.789, 38654321.987, 38789012.345],
            "Y坐标": [54321.678, 65432.123, 76543.456],
            "带号": [38, 38, 39],
            "备注（可选）": ["示例点1", "示例点2", "示例点3"]
        })
